...
export const createSessionService = async(userId: string, req: CustomRequest) => {
    const exitingActiveSession = await UserSessionModel.find({ user: userId, active: true })
            // logout from all devices
            if (exitingActiveSession.length > 0) {
                await UserSessionModel.updateMany({ user: userId, active: true }, { active: false })
            }
